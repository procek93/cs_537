...
copyuvm(pde_t *pgdir, uint sz, uint st)
{
  ...
  if((d = setupkvm()) == 0)
    return 0;
  //start crawling memory and copying page table where code section
  //first appears which is at x1000 (4096), not 0
  //*user code given 

  //*****PART B MOD.9**********************************************//
  /*this was also changed in part A, so that i = 4096... this is where
  *the page will begin. This code should still be good, because we iterate
  *from begining of our first page (code), and if heap has been allocated, we'll
  *copy the heap pages too.
  */
  for(i = 4096; i < sz; i += PGSIZE){
	cprintf("***************************I IN COPYUVM LOOP = %d\n", i);
  ...
  /**BUT WE NEED A LOOP TO ALSO COPY THE STACK!!!!!*/
